...
func (r *Reconciler) ReconcileKind(ctx context.Context, channel *messagingv1beta1.KafkaChannel) reconciler.Event {
	...
	channelConfig, err := r.getChannelContractResource(ctx, topic, channel, secret, topicConfig)
	if err != nil {
		return statusConditionManager.FailedToGetConfig(err)
	}
...
func (r *Reconciler) reconcileSubscribers(ctx context.Context, channel *messagingv1beta1.KafkaChannel, topicName string, bootstrapServers []string) error {
	...
	for i := range channel.Spec.Subscribers {
		...
		err := r.reconcileConsumerGroup(ctx, channel, s, topicName, bootstrapServers)
		if err != nil {
...
func (r Reconciler) reconcileConsumerGroup(ctx context.Context, channel *messagingv1beta1.KafkaChannel, s *v1.SubscriberSpec, topicName string, bootstrapServers []string) error {
	...
	if apierrors.IsNotFound(err) {
		...
		if err != nil && !apierrors.IsAlreadyExists(err) {
			return fmt.Errorf("failed to create consumer group %s/%s: %w", newcg.GetNamespace(), newcg.GetName(), err)
		}
	...
	if _, err = r.InternalsClient.InternalV1alpha1().ConsumerGroups(cg.GetNamespace()).Update(ctx, newCg, metav1.UpdateOptions{}); err != nil {
		return fmt.Errorf("failed to update consumer group %s/%s: %w", newCg.GetNamespace(), newCg.GetName(), err)
	}
