...
type KafkaSinkLister interface {
	...
	List(selector labels.Selector) (ret []*eventingv1.KafkaSink, err error)
...
type kafkaSinkLister struct {
	listers.ResourceIndexer[*eventingv1.KafkaSink]
}

// NewKafkaSinkLister returns a new KafkaSinkLister.
func NewKafkaSinkLister(indexer cache.Indexer) KafkaSinkLister {
	return &kafkaSinkLister{listers.New[*eventingv1.KafkaSink](indexer, eventingv1.Resource("kafkasink"))}
}

// KafkaSinks returns an object that can list and get KafkaSinks.
func (s *kafkaSinkLister) KafkaSinks(namespace string) KafkaSinkNamespaceLister {
	return kafkaSinkNamespaceLister{listers.NewNamespaced[*eventingv1.KafkaSink](s.ResourceIndexer, namespace)}
}
...
type KafkaSinkNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*eventingv1.KafkaSink, error)
