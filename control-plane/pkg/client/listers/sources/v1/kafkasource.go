...
// List lists all KafkaSources in the indexer.
func (s *kafkaSourceLister) List(selector labels.Selector) (ret []*v1.KafkaSource, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.KafkaSource))
	})
	return ret, err
}
...
// List lists all KafkaSources in the indexer for a given namespace.
func (s kafkaSourceNamespaceLister) List(selector labels.Selector) (ret []*v1.KafkaSource, err error) {
...
// Get retrieves the KafkaSource from the indexer for a given namespace and name.
func (s kafkaSourceNamespaceLister) Get(name string) (*v1.KafkaSource, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("kafkasource"), name)
	}
	return obj.(*v1.KafkaSource), nil
}
