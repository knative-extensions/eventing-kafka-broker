...
// controller reconciling v1.KafkaChannel.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.KafkaChannel. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.KafkaChannel) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.KafkaChannel.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.KafkaChannel. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.KafkaChannel) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.KafkaChannel if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.KafkaChannel.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.KafkaChannel) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.KafkaChannel) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.KafkaChannel resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister messagingv1.KafkaChannelLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1.KafkaChannel, desired *v1.KafkaChannel) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.MessagingV1().KafkaChannels(desired.Namespace)
		...
		updater := r.Client.MessagingV1().KafkaChannels(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.KafkaChannel, desiredFinalizers sets.Set[string]) (*v1.KafkaChannel, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1.KafkaChannel, desiredFinalizers sets.Set[string]) (*v1.KafkaChannel, error) {
	...
	patcher := r.Client.MessagingV1().KafkaChannels(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1.KafkaChannel, desiredFinalizers sets.Set[string]) (*v1.KafkaChannel, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.KafkaChannel) (*v1.KafkaChannel, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.KafkaChannel, reconcileEvent reconciler.Event) (*v1.KafkaChannel, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.MessagingV1().KafkaChannels(resource.Namespace)
