...
// controller reconciling v1.KafkaSource.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.KafkaSource. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.KafkaSource) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.KafkaSource.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.KafkaSource. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.KafkaSource) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.KafkaSource if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.KafkaSource.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.KafkaSource) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.KafkaSource) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.KafkaSource resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister sourcesv1.KafkaSourceLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1.KafkaSource, desired *v1.KafkaSource) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.SourcesV1().KafkaSources(desired.Namespace)
		...
		updater := r.Client.SourcesV1().KafkaSources(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.KafkaSource, desiredFinalizers sets.Set[string]) (*v1.KafkaSource, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1.KafkaSource, desiredFinalizers sets.Set[string]) (*v1.KafkaSource, error) {
	...
	patcher := r.Client.SourcesV1().KafkaSources(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1.KafkaSource, desiredFinalizers sets.Set[string]) (*v1.KafkaSource, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.KafkaSource) (*v1.KafkaSource, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.KafkaSource, reconcileEvent reconciler.Event) (*v1.KafkaSource, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.SourcesV1().KafkaSources(resource.Namespace)
