...
public class ReceiverVerticleTest {
        ...
        public void setUpHTTP(final Vertx vertx, final VertxTestContext testContext) {
                ...
                ReceiverVerticleTest.mockProducer = new MockProducer<>(true, new StringSerializer(),
                                new CloudEventSerializerMock());
                ReactiveKafkaProducer<String, CloudEvent> producer = new MockReactiveKafkaProducer<>(mockProducer);

                store = new IngressProducerReconcilableStore(AuthProvider.noAuth(), new Properties(),
                                properties -> producer);
                ...
                tc.requestSender
                                ...
                                .onSuccess(response -> vertx.setTimer(
                                                ...
                                                ignored -> context.verify(() -> {
                                                        ...
                                                        final var expectedCount = (double) tc.badRequestCount
                                                                        + (double) tc.produceEventCount;

                                                        final var defaultCounter = Counter.builder(Metrics.EVENTS_COUNT)
                                                                        .register(Metrics.getRegistry());
                                                        Counter counter = defaultCounter;
                                                        try {
                                                                if (expectedCount > 0) {
                                                                        counter = registry.get(Metrics.EVENTS_COUNT)
                                                                                        .counters().stream()
                                                        ...
                                                        if (tc.expectedDispatchLatency) {
                                                                final var eventDispatchLatency = registry
                                                                ...
                                                                assertThat(eventDispatchLatency.totalAmount())
                                                                                .isGreaterThan(0);
                                                                assertThat(eventDispatchLatency.mean())
                                                                                .isGreaterThan(0);
                                                        }
        ...
        public void secretFileUpdated() throws InterruptedException {
                ...
                // Check the logger to see if the message "The secret volume is updated" is
                // printed by using
                // logger
        ...
        private static List<TestCase> getValidNonValidEvents() {
                return Arrays.asList(
                                ...
                                new TestCase(
                                                ...
                                                request -> request
                                                                .sendBuffer(Buffer.buffer("this is not a cloud event")),
                                                BAD_REQUEST.code(),
                                                DataPlaneContract.Resource.newBuilder()
                                                                .setUid("1")
                                                                .addTopics("topic")
        ...
        private static Function<HttpRequest<Buffer>, Future<HttpResponse<Buffer>>> probeRequestSender(
                ...
                return request -> request.method(HttpMethod.GET)
                                .putHeader(
                                                ControlPlaneProbeRequestUtil.PROBE_HEADER_NAME,
                                                ControlPlaneProbeRequestUtil.PROBE_HEADER_VALUE)
                                .send();
        }
