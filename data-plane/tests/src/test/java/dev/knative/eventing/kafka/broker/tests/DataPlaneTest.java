...
public class DataPlaneTest {
  ...
  public void execute(final Vertx vertx, final VertxTestContext context)
    ...
    final var expectedResponseEventService2 =
    ...
    final var expectedResponseEventService1 =
        CloudEventBuilder.v1()
            .withId(UUID.randomUUID().toString())
  ...
  public static void teardown(final Vertx vertx) throws ExecutionException, InterruptedException {
    ...
    vertx
        .undeploy(consumerDeployerVerticle.deploymentID())
        .toCompletionStage()
        .toCompletableFuture()
        .get();

    vertx.undeploy(receiverVerticle.deploymentID()).toCompletionStage().toCompletableFuture().get();
    ...
    final var verticle =
        new ReceiverVerticle(
            ...
            new IngressRequestHandlerImpl(
                StrictRequestToRecordMapper.getInstance(), Metrics.getRegistry()),
            SECRET_VOLUME_PATH);
