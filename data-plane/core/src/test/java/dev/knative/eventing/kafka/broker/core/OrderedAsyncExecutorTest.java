...
public class OrderedAsyncExecutorTest {
    ...
    public void shouldExecuteInOrder(final long millis, final int tasks, final Vertx parentVertx)
        ...
        for (int i = 0; i < tasks; i++) {
            final var n = i;
            vertx.runOnContext(
                    (v) -> asyncExecutor.offer(generateTask(vertx, random, millis, n, tasksLatch, executed)));
        }
    ...
    private Supplier<Future<?>> generateTask(
            Vertx vertx, Random random, long millis, int i, CountDownLatch latch, List<Integer> executed) {
        if (millis == 0) {
            return () -> {
                executed.add(i);
                latch.countDown();
