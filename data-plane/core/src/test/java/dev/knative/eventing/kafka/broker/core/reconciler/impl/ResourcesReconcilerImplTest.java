...
class ResourcesReconcilerImplTest {
  ...
  void reconcileIngressAddNewResourcesWithoutIngress() {
    new ResourceReconcilerTestRunner()
        ...
        .reconcile(List.of(baseResource("1-1234").build(), baseResource("2-1234").build()))
        .run();
  }
        ...
        .reconcile(
            List.of(
                baseResource("1-1234")
                    ...
                    .addEgresses(
                        baseEgress("bbb")
                            .setFilter(Filter.newBuilder().putAttributes("id", "hello")))
                    .addEgresses(egress("ccc"))
                    .build()))
        .expect()
        .updatedEgress("bbb")
  ...
  private Resource.Builder baseResource(final String uid) {
    return Resource.newBuilder().setUid(uid).addTopics("hello.topic");
  }

  private Egress.Builder baseEgress(final String uid) {
    return Egress.newBuilder().setUid(uid).setConsumerGroup("hello");
  }

  private Egress egress(final String uid) {
