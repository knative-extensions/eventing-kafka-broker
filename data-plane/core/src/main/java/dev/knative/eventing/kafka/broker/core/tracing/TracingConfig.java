...
public final class TracingConfig {
    ...
    TracingConfig(
            final Backend backend, final String url, final float samplingRate, final HeadersFormat headersFormat) {
    ...
    public OpenTelemetrySdk setup() {
        ...
        tracerProviderBuilder.setSampler(Sampler.parentBased(Sampler.traceIdRatioBased(getSamplingRate())));
        ...
        if (getBackend().equals(Backend.ZIPKIN)) {
            if (logger.isDebugEnabled()) {
                ...
                tracerProviderBuilder.addSpanProcessor(SimpleSpanProcessor.create(zipkinExporter(this)));
            } else {
    ...
    public String toString() {
        return "TracingConfig{" + "backend="
                + backend + ", url='"
                + url + '\'' + ", samplingRate="
                + samplingRate + ", headersFormat="
                + headersFormat + '}';
    }
    ...
    private static SpanExporter zipkinExporter(TracingConfig tracingConfig) {
        return ZipkinSpanExporter.builder().setEndpoint(tracingConfig.getUrl()).build();
    }
        ...
        if (backend.equals(Backend.ZIPKIN)) {
            ...
            if (Files.exists(headersFormatPath)) {
                try (final var headersFormatFile = new FileInputStream(headersFormatPath.toString())) {
                    ...
                    if (parsed.equals(HeadersFormat.B3_MULTI_HEADER) || parsed.equals(HeadersFormat.B3_SINGLE_HEADER)) {
