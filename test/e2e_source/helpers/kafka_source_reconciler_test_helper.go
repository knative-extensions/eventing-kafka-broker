...
func testKafkaSourceReconciler(c *testlib.Client, name string, doAction func(c *testlib.Client), expectedStatuses sets.String, wantRADepCount int) {
	...
	if err := CheckKafkaSourceState(context.Background(), c, rtKafkaSourceName, func(ks *sourcesv1beta1.KafkaSource) (bool, error) {
	...
	if err := CheckRADeployment(context.Background(), c, rtKafkaSourceName, func(deps *appsv1.DeploymentList) (bool, error) {
...
func createKafkaSourceWithSinkMissing(c *testlib.Client) {
	MustCreateTopic(c, KafkaClusterName, KafkaClusterNamespace, rtKafkaTopicName, 10)
...
func deleteChannel(c *testlib.Client) {
	contribtestlib.DeleteResourceOrFail(context.Background(), c, rtChannelName, ImcGVR)
}
